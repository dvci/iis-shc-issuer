...
class HealthCardController < ApplicationController
  ...
  def create
    ...
    if vaccine_group == 'COVID-19' 
      jws = issuer.issue_jws(fhir_bundle, type: HealthCards::COVIDHealthCard)
    else
      jws = issuer.issue_jws(fhir_bundle, type: HealthCards::HealthCard)
    end 
