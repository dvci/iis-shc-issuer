...
class ImmunizationTest < ActiveSupport::TestCase
  test 'lookup_vaccine_display with single tradename' do
    imm_model = Immunization.new(
      FHIR.from_contents(File.read('test/fixtures/files/immunization.json'))
    )
  ...
  test 'lookup_vaccine_display without tradename' do
    imm_model = Immunization.new(
      FHIR.from_contents(File.read('test/fixtures/files/immunization_no_tradename.json'))
    )
  ...
  test 'lookup_vaccine_display multiple tradenames' do
    imm_model = Immunization.new(
      FHIR.from_contents(File.read('test/fixtures/files/immunization_multiple_tradenames.json'))
    )
  ...
  test 'lookup_vaccine_display unknown cvx' do
    imm_model = Immunization.new(
      FHIR.from_contents(File.read('test/fixtures/files/immunization_invalid_cvx.json'))
    )
